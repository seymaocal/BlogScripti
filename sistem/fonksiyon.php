...
function pagination($s, $ptotal, $url){
    ...
    if($ptotal < 2){
        ...
        if($s > 4){
            $prev  = $s - 1;
            echo '<li class="page-item"><a class="pgn__prev" href="'.$site.'/'.$url.'1" ><i class="fa fa-angle-left"></i></a></li>';
            echo '<li class="page-item"><a class="pgn__prev" href="'.$site.'/'.$url.($s-1).'" ><</a></li>';
        }

        for($i = $s - $forlimit; $i < $s + $forlimit + 1; $i++){
            if($i > 0 && $i <= $ptotal){
                if($i == $s){
                    echo '<li class="page-item active"><a class="pgn__num current"  href="#">'.$i.'</a></li>';
                }else{
                    echo '<li class="page-item"><a class="pgn__num " href="'.$site.'/'.$url.$i.'" >'.$i.'</a></li>';
                }
            }
        }
        ...
        if($s <= $ptotal - 4){
            $next  = $s + 1;
            echo '<li class="page-item"><a class="pgn__next" href="'.$site.'/'.$url.$next.'" > <i class="fa fa-angle-right"></i></a></li>';
            echo '<li class="page-item"><a class="pgn__next" href="'.$site.'/'.$url.$ptotal.'" >»</a></li>';
        }
    }
